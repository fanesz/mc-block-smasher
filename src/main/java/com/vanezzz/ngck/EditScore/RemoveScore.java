...
public class RemoveScore implements CommandExecutor {
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage("§e>>§f Wrong Args!, /scoreremove <player> <score>");
            return true;
        }
        ...
        for (lbData playerList : BreakCounterLB.dataList) {
            if(playerList.nick.equals(target.getName())) {
                ...
                sender.sendMessage("§e>> §7§oSuccessfully remove §c§o"+args[1]+" score §7§o from " + target.getName() + "  (score: "+String.valueOf(playerList.count)+")");
                target.sendMessage("§e>> §7§oYou got §c§o"+args[1]+" score§7§o removed!" + "§7§o (score: "+String.valueOf(playerList.count)+")");
            }
        }
